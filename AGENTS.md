...
- Keep commits and documentation in the repository's normal engineering style. Do not add tool-generated attribution footers, automation banners, or provenance boilerplate unless the user explicitly asks for them.
...
- Never execute `terraform apply` in this repository. Agents may run read-only or planning commands such as `terraform init -backend=false`, `terraform validate`, `terraform test`, `terraform plan`, `terraform providers schema`, and `terraform graph`, but every real apply must be run manually by the user outside the agent session.
- Never use `gcloud` to create, update, delete, patch, enable, import, or otherwise mutate Google Cloud resources for this repo. `gcloud` is allowed only for read-only inspection such as `list` and `describe`, unless the user explicitly changes this guardrail for a specific action.
- Test resources created for this repo must use the `dgc` prefix in resource IDs/names and must include tracking labels through `default_labels` or resource labels. Use labels such as `managed_by = "terraform"`, `test_owner = "dgc"`, `test_scope = "oracle-database-at-gcp"`, and `cleanup = "manual"` unless the user specifies different tracking values.
...
- ODB Network and ODB Subnet `labels` are intentionally included in `lifecycle.ignore_changes` because the current Google provider plans replacement for label-only changes; treat those labels as creation-time tracking metadata.
- Keep ODB networking hardening aligned with ADB/ExaDB: validate Google label syntax, reject whitespace-only project/location/GCP Oracle zone values, and require `gcp_oracle_zone` or `default_gcp_oracle_zone` for ODB Networks. Do not reintroduce standalone `terraform_data` uniqueness checks; duplicate IDs are left to the Google provider/API to stay closer to OCI module style.
...
- Cloud VM Cluster `labels` are intentionally included in `lifecycle.ignore_changes` because the current Google provider plans replacement for label-only changes; treat those labels as creation-time tracking metadata.
- Cloud VM Cluster `properties.db_server_ocids` is optional+computed in the Google provider schema, so `null` can plan, but real VM Cluster creation is certified with explicit DB server OCIDs. Keep examples/templates on the explicit-OCID path and leave `null` only as an advanced, already-validated environment option.
...
- `examples/existing-odb-network` owns file-path consumption through:
  - `gcp_odb_networks_dependency_file_path`
  - `gcp_odb_subnets_dependency_file_path`
- `examples/vision` is the end-to-end example and composes `modules/odb-networking` with `modules/adb`.
- `examples/existing-odb-network` is the multi-stack consumer example.
...
- Admin passwords are validated at plan time: 12-30 characters, at least one uppercase letter, one lowercase letter, one number, no double quotes, and no `admin` substring in any casing.
- ADB `labels` are intentionally included in `lifecycle.ignore_changes` because the current Google provider plans replacement for label-only changes; treat those labels as creation-time tracking metadata.
- ADB outputs are extended for downstream handoff and include Google identifiers, OCI identifiers and parsed OCI URL fields, connection strings and URLs, private endpoint details, SQL Web Developer URL, Data Guard/peer metadata, and lifecycle state.
- Additional ADB plan-time validations cover `database` format, Google label syntax, non-empty project/location values, `operations_insights_state` enum values, IPv4-only `private_endpoint_ip`, and non-empty optional provider string fields. Do not reintroduce standalone `terraform_data` uniqueness checks; duplicate names are left to the Google provider/API to stay closer to OCI module style.
...
When touching provider-sensitive behavior, inspect the installed Google provider schema when possible:

```sh
cd modules/<module>
terraform providers schema -json > /tmp/<module>-provider-schema.json
```

Then query the generated schema with `jq`, for example:

```sh
jq '.provider_schemas["registry.terraform.io/hashicorp/google"].resource_schemas | keys | map(select(startswith("google_oracle_database_")))' /tmp/exadb-provider-schema.json
```

Known Codex sandbox issue: `terraform providers schema -json` may fail even when `terraform validate` and `terraform test` pass. The typical error is:

```text
plugin init error: listen unix ... bind: operation not permitted
Failed to load plugin schemas
```

Terraform provider plugins use a temporary Unix socket for RPC, and the sandbox can block `bind` on that socket. If this happens, do not change provider constraints or reinstall providers as a workaround. Rerun the same schema command with sandbox escalation/approval, writing the schema to `/tmp`, and then inspect the generated JSON.

Example schema checks used for this repo:

```sh
jq '{
  odb_network_attrs:(.provider_schemas["registry.terraform.io/hashicorp/google"].resource_schemas["google_oracle_database_odb_network"].block.attributes|keys),
  odb_subnet_attrs:(.provider_schemas["registry.terraform.io/hashicorp/google"].resource_schemas["google_oracle_database_odb_subnet"].block.attributes|keys),
  cloud_exadata_attrs:(.provider_schemas["registry.terraform.io/hashicorp/google"].resource_schemas["google_oracle_database_cloud_exadata_infrastructure"].block.attributes|keys),
  cloud_vm_cluster_attrs:(.provider_schemas["registry.terraform.io/hashicorp/google"].resource_schemas["google_oracle_database_cloud_vm_cluster"].block.attributes|keys)
}' /tmp/exadb-provider-schema.json
```

For ADB-specific changes, inspect the Autonomous Database resource directly:

```sh
jq '{
  adb_attrs:(.provider_schemas["registry.terraform.io/hashicorp/google"].resource_schemas["google_oracle_database_autonomous_database"].block.attributes|keys),
  adb_property_attrs:(.provider_schemas["registry.terraform.io/hashicorp/google"].resource_schemas["google_oracle_database_autonomous_database"].block.block_types.properties.block.attributes|keys)
}' /tmp/adb-provider-schema.json
```
...
- admin password policy is validated for ADB;
- duplicate resource IDs and ADB duplicate `database` names are left to the provider/API, matching OCI module style;
- Google label syntax is validated where exposed;
- provider enum fields and non-empty optional provider strings are validated where exposed;
- ADB extended output JSON includes OCI, connection, endpoint, and peer metadata fields.
