...
When `output_path` is set, apply writes `gcp_odb_networks_output.json` and `gcp_odb_subnets_output.json` to that directory. The consumer stack (`examples/cluster`) can receive the resource IDs as inline maps via Terragrunt, `terraform_remote_state`, or CI/CD — or pass the JSON file paths directly for standalone deployments.
